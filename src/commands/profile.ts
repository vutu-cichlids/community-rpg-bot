...
import { MAX_LEVEL, xpNeededForLevel } from "../lib/xp";
  ...
  async execute(interaction) {
    ...
    const embed = baseEmbed(`${classDef?.emoji ?? "🌱"} Hồ sơ của ${target.username}`, COLORS.primary)
      ...
      .addFields(
        ...
        { name: "Level", value: isMaxLevel ? `${MAX_LEVEL} 🏆 MAX` : `${user.level}/${MAX_LEVEL}`, inline: true },
        { name: "Clan", value: clan ? clan.name : "Không có", inline: true },
        {
          name: "XP",
          value: isMaxLevel ? "Đã đạt cấp tối đa!" : `${xpBar(user.xp, needed)} ${user.xp}/${needed}`,
          inline: false,
        },
